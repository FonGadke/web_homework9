    ...
    export default {
        ...
        components: {
            ...
            this.$root.$on("onRegister", (login, name) => {
                let users = Object.values(this.users).filter(u => u.login === login);
                if (!(/^[a-z]+$/.test(login))) {
                    this.$root.$emit("onRegisterValidationError", "Login must contains only latin lowercase letters")
                } else if (login.length < 3 || login.length > 16 ) {
                    this.$root.$emit("onRegisterValidationError", "Login length must be from 3 to 16")
                } else if (name.length < 1 || name.length > 32) {
                    this.$root.$emit("onRegisterValidationError", "Name length must be from 1 to 32")
